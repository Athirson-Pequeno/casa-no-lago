...
// Adicionar tag a um quarto
const adicionarTagAoQuarto = async (req, res) => {
    try {
        const { quartoId, tagId } = req.body;
        if (!quartoId || !tagId) return res.status(400).json({ erro: "quartoId e tagId são obrigatórios." });
